...
def load_model_pt(model: torch.nn.Module, path: str, strict: bool = True, force: bool = False) -> dict[str, Any]:
    ...
    if 'model' not in data:
        print("The checkpoint file is missing the 'model' key.")
        model_weights = data
    else:
        model_weights = data['model']

    if 'type' not in data:
        print("The checkpoint file does not contain a 'type' key. Skipping type validation.")
    ...
    if force:
        ...
        print(f"Loaded model with dynamic mismatch resolution from {os.path.abspath(path)}.")
    elif 'type' in data and not str(type(model)) == data['type']:
        # Validate model type if 'type' key exists
        print(f"The type in the file ({data['type']}) is different from the passed model ({type(model)}).")
        model.load_state_dict(model_weights, strict=strict)
        print(f"Loaded model from {os.path.abspath(path)} with potential mismatches (strict={strict}).")
    else:
        # Normal weight loading
        model.load_state_dict(model_weights, strict=strict)
        print(f"Loaded model from {os.path.abspath(path)} with strict={strict}.")
